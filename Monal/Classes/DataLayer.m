...
NSString *const KMessageTypeReply = @"^$$%^";//^$$%
NSString *const KMessageDecrypt = @"Decrypt_Message";
NSString *const kMessageTypeUrl = @"Url";
NSString *const kMessageTypeImageCaption = @"%cap-t-ion";
NSString *const kMessageTypeFiletransfer = @"Filetransfer";

NSString *const kMessageTypeCreateGroup = @"#^%$%###^%$%##";
NSString *const kMessageTypeAddMember = @"#^%%#$##^%%#$#";
NSString *const kMessageTypeChangedGroupSubjectName = @"#^$#%%##^$#%%#?changed group subject name !";
NSString *const kMessageTypeChangedGroupInfo = @"#^$#%%##^$#%%#?changed group info !";
...
-(NSMutableArray*) listBuddiesForAccount:(NSString*) accountNo
{
    // FROM buddylist WHERE account_id=? AND buddy_name=?
    return [self.db idReadTransaction:^{
        return [self.db executeReader:@"SELECT * FROM buddylist WHERE account_id=? AND muc=0;" andArguments:@[accountNo]];
    }];
}
