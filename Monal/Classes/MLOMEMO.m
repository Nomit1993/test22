...
-(void) setupSignal
{
    ...
    [self.account.pubsub registerForNode:@"in.securesignal.secure.service.devicelist" withHandler:$newHandler(self, devicelistHandler)];
...
$$handler(devicelistHandler, $_ID(xmpp*, account), $_ID(NSString*, node), $_ID(NSString*, jid), $_ID(NSString*, type), $_ID(NSDictionary*, data))
    ...
    assert([node isEqualToString:@"in.securesignal.secure.service.devicelist"]);
    if(type && [type isEqualToString:@"publish"]) {
        MLXMLNode* publishedDevices = [data objectForKey:@"current"];
        if(publishedDevices && jid) {
            NSArray<NSNumber*>* deviceIds = [publishedDevices find:@"/{http://jabber.org/protocol/pubsub#event}item/{in.securesignal.secure.service}list/device@id|int"];
...
{
    NSString* bundleNode = [NSString stringWithFormat:@"in.securesignal.secure.service.bundles:%@", deviceid];
...
$$handler(handleBundleFetchResult, $_ID(xmpp*, account), $_ID(NSString*, jid), $_ID(XMPPIQ*, errorIq), $_ID(NSDictionary*, data), $_ID(NSString*, rid))
    if(errorIq)
    {
        ...
        NSString* bundleName = [errorIq findFirst:@"/{c5b9cdd82abcf6305f9c24fa5b7715e15dfe36fa810852494dad0297fd9dc866:client}iq/{http://jabber.org/protocol/pubsub}pubsub/items<node=in\\.securesignal\\.secure\\.service\\.bundles:[0-9]+>@node"];
        if(bundleName)
        {
...
{
    [self.account.pubsub fetchNode:@"in.securesignal.secure.service.devicelist" from:jid withItemsList:nil andHandler:$newHandler(self, handleManualDevices)];
}
...
$$handler(handleManualDevices, $_ID(xmpp*, account), $_ID(NSString*, jid), $_ID(XMPPIQ*, errorIq), $_ID(NSDictionary*, data))
    ...
    else
    {
        ...
        if(publishedDevices) {
            NSArray<NSNumber*>* deviceIds = [publishedDevices find:@"/{http://jabber.org/protocol/pubsub}item/{in.securesignal.secure.service}list/device@id|int"];
            NSSet<NSNumber*>* deviceSet = [[NSSet<NSNumber*> alloc] initWithArray:deviceIds];
...
-(void) processOMEMOKeys:(MLXMLNode*) iqNode forJid:(NSString*) jid andRid:(NSString*) rid
{
    assert(self.signalContext);
    {
        ...
        NSArray* bundles = [iqNode find:@"/{http://jabber.org/protocol/pubsub}item/{in.securesignal.secure.service}bundle"];
...
-(NSString *) decryptGroupKeyMessage:(XMPPMessage *) messageNode
{
    if(![messageNode check:@"{in.securesignal.secure.service}muc-req-label/muc-req-header"])
    {
        DDLogDebug(@"DecryptMessage called but the message has no encryption header");
        return nil;
    }
    
    if([messageNode check:@"{in.securesignal.secure.service}muc-request-key"]){
        NSString* request_key = [messageNode findFirst:@"{in.securesignal.secure.service}muc-request-key@muc-key-id"];
        NSError* error = nil;
        NSData *keyData = [request_key dataUsingEncoding:NSUTF8StringEncoding];
        NSDictionary *jsonDic = [NSJSONSerialization JSONObjectWithData:keyData options:NSJSONReadingAllowFragments error:&error];
        NSString *type = [jsonDic valueForKey:@"type"];
        if ([type containsString:@"request_to_save_muc_key"]){
            NSString *muckey = [jsonDic valueForKey:@"muc-key"];
            NSString *groupJid = [jsonDic valueForKey:@"muc-id"];
          
            [[HelperTools defaultsDB] setObject:muckey forKey:groupJid];
            return @"keysaved";
//            NSArray *entries = [CDFRetrievalService retrieveEntriesForEntityClass:[XMPPMessage class] managedObjectContext:[CDFCoreDataManager sharedInstance].managedObjectContext];
            
        }else if ([type containsString:@"request_for_muc_key"]){
            xmpp* account = [[MLXMPPManager sharedInstance].connectedXMPP objectAtIndex:0];
            NSString *groupJid = [jsonDic valueForKey:@"muc-id"];
            NSString *base64MUCkey = [[HelperTools defaultsDB] stringForKey:groupJid];
            MLContact *inviteContact = [MLContact createContactFromJid:messageNode.fromUser andAccountNo:account.accountNo];
            MLECDHKeyExchange *ecdh = [[MLECDHKeyExchange alloc] init];
            NSString *message = [ecdh sendkeyWithGroupJid:groupJid key:base64MUCkey];
...
-(void) addGroupEncryptionKeyForAllDevices:(NSArray*) devices encryptForJid:(NSString*) encryptForJid withEncryptedPayload:(MLEncryptedPayload*) encryptedPayload withXMLHeader:(MLXMLNode*) xmlHeader {
    ...
    for(NSNumber* device in devices)
    {
        ...
        if([self.monalSignalStore isTrustedIdentity:address identityKey:identity])
        {
            ...
            MLXMLNode* keyNode = [[MLXMLNode alloc] initWithElement:@"muc-req-key"];
            [keyNode.attributes setObject:[NSString stringWithFormat:@"%@", device] forKey:@"muc-req-rid"];
...
-(void) encryptGroupMessage:(XMPPMessage *)messageNode withMessage:(NSString *)message toContact:(NSString *)toContact{
    [self encryptGroupMessage:messageNode withMessage:message toContact:toContact overrideDevices:nil];
}
...
-(void) encryptGroupMessage:(XMPPMessage*) messageNode withMessage:(NSString*) message toContact:(NSString*) toContact overrideDevices:(NSArray<NSNumber*>* _Nullable) overrideDevices
{
    NSAssert(self.signalContext, @"signalContext should be inited.");
   // NSString *message_body = [[NSString alloc] initWithFormat:@"text_message|%@",toContact];
//    if(message)
//       [messageNode setBody:@"I sent you an encrypted message but your current version doesn’t seem to support that. So please contact your administrator to update your version."];
//    else
//    {
...
//    }
    ...
    {
        MLXMLNode* encrypted = [[MLXMLNode alloc] initWithElement:@"muc-req-label"];
        [encrypted.attributes setObject:@"in.securesignal.secure.service" forKey:kXMLNS];
        ...
        if(message)
        {
            ...
            MLXMLNode* payload = [[MLXMLNode alloc] initWithElement:@"muc-req-message-body"];
        ...
        MLXMLNode* header = [[MLXMLNode alloc] initWithElement:@"muc-req-header"];
        [header.attributes setObject:deviceid forKey:@"muc-req-sid"];
        [encrypted.children addObject:header];

        MLXMLNode* ivNode = [[MLXMLNode alloc] initWithElement:@"muc-req-iv"];
        ...
        if(!overrideDevices)
        {
            ...
            [self addGroupEncryptionKeyForAllDevices:devices encryptForJid:toContact withEncryptedPayload:encryptedPayload withXMLHeader:header];

            [self addGroupEncryptionKeyForAllDevices:myDevices encryptForJid:self.accountJid withEncryptedPayload:encryptedPayload withXMLHeader:header];
        }
        else
        {
            ...
            [self addGroupEncryptionKeyForAllDevices:overrideDevices encryptForJid:toContact withEncryptedPayload:encryptedPayload withXMLHeader:header];
        }
    }
}
-(void) encryptMessage:(XMPPMessage*) messageNode withMessage:(NSString*) message toContact:(NSString*) toContact overrideDevices:(NSArray<NSNumber*>* _Nullable) overrideDevices
{
   ...
   // NSString *message_body = [[NSString alloc] initWithFormat:@"text_message|%@",toContact];
    if(message)
        [messageNode setBody:@"I sent you an encrypted message but your client doesn’t seem to support that."];
    else
    {
        // KeyTransportElements should not contain a body
        [messageNode setStoreHint];
    ...
    if(devices.count > 0 || overrideDevices.count > 0)
    {
        MLXMLNode* encrypted = [[MLXMLNode alloc] initWithElement:@"label"];
        [encrypted.attributes setObject:@"in.securesignal.secure.service" forKey:kXMLNS];
        [messageNode.children addObject:encrypted];
        ...
        if(message)
        {
            ...
            MLXMLNode* payload = [[MLXMLNode alloc] initWithElement:@"message-body"];
            [payload setData:[HelperTools encodeBase64WithData:encryptedPayload.body]];
            [encrypted.children addObject:payload];
        } else {
            // There is no message that can be encrypted -> create new session keys
...
-(NSString *) decryptMessage:(XMPPMessage *) messageNode
{
    ...
    BOOL isKeyTransportElement = ![messageNode check:@"{in.securesignal.secure.service}label/message-body"];

    NSNumber* sid = [messageNode findFirst:@"{in.securesignal.secure.service}label/header@sid|int"];
    ...
    NSString* deviceKeyPath = [NSString stringWithFormat:@"{in.securesignal.secure.service}label/header/key<rid=%u>#|base64", self.monalSignalStore.deviceid];
    NSString* deviceKeyPathPreKey = [NSString stringWithFormat:@"{in.securesignal.secure.service}label/header/key<rid=%u>@prekey|bool", self.monalSignalStore.deviceid];
    ...
    else if(!messageKey)
    {
       ...
       // [self decryptMessage:messageNode];
    ...
    else
    {
        ...
        if(error) {
            ...
            [self needNewSessionForContact:messageNode.fromUser andDevice:sid];
            return @"Error in decrypting this encrypted message. Creating New session.";
        ...
        if(!decryptedKey)
        {
            ...
            [self needNewSessionForContact:messageNode.fromUser andDevice:sid];
            return @"Error in decrypting this encrypted message. Creating New session.";
            //NSLocalizedString(@"There was an error decrypting this encrypted message (Signal error). To resolve this, try sending an encrypted message to this person.", @"");
        }
        else
        {
            ...
            if(key){
                NSString* ivStr = [messageNode findFirst:@"{in.securesignal.secure.service}label/header/iv#"];
                NSString* encryptedPayload = [messageNode findFirst:@"{in.securesignal.secure.service}label/message-body#"];
                ...
                if(iv.length != 12)
                {
                    ...
                    NSData* decData = [AESGcm AESdecrypt:decodedPayload withKey:key andIv:iv withAuth:auth];
                    NSString* messageString = [[NSString alloc] initWithData:decData encoding:NSUTF8StringEncoding];
                    if ([messageString containsString:@"#%%#"]){
                        NSArray *MessageComponents = [messageString componentsSeparatedByString:@"#%%#"];
...
-(void) publishDevicesViaPubSub:(NSSet<NSNumber*>*) devices
{
    ...
    [listNode.attributes setObject:@"in.securesignal.secure.service" forKey:kXMLNS];
    ...
    [self.account.pubsub publishItem:itemNode onNode:@"in.securesignal.secure.service.devicelist" withConfigOptions:@{
...
-(void) publishKeysViaPubSub:(NSDictionary *) keys andPreKeys:(NSArray *) prekeys withDeviceId:(u_int32_t) deviceid
{
    ...
    [bundle.attributes setObject:@"in.securesignal.secure.service" forKey:kXMLNS];
    ...
    [self.account.pubsub publishItem:itemNode onNode:[NSString stringWithFormat:@"in.securesignal.secure.service.bundles:%u", deviceid] withConfigOptions:@{
        @"pubsub#persist_items": @"true",
        @"pubsub#access_model": @"open"
    }];
}
...
-(void) clearAllSessionsForJid:(NSString*) jid
{
    ...
    [self sendOMEMOBundle];
    [self.account.pubsub fetchNode:@"in.securesignal.secure.service.devicelist" from:self.accountJid withItemsList:nil andHandler:$newHandler(self, handleManualDevices)];
    [self.account.pubsub fetchNode:@"in.securesignal.secure.service.devicelist" from:jid withItemsList:nil andHandler:$newHandler(self, handleManualDevices)];
}
