...
+(NSDictionary*) pushServer
{
    if (@available(iOS 13.0, *))        // for ios 13 onwards
        return @{
            @"jid": @"push.xmppserver.com",
            @"url": @"https://push.xmppserver.com:5269/push_appserver",
        };
    else                                // for ios 12
        return @{
            @"jid": @"push.xmppserver.com",
...
+(NSString*) extractXMPPError:(XMPPStanza*) stanza withDescription:(NSString*) description
{
    ...
    NSString* errorReason = [stanza findFirst:@"error/{in:secure:signal:xml:ns:xmpp-stanzas}!text$"];
    NSString* errorText = [stanza findFirst:@"error/{in:secure:signal:xml:ns:xmpp-stanzas}text#"];
...
+(NSSet*) getOwnFeatureSet
{
    ...
    dispatch_once(&onceToken, ^{
        NSArray* featuresArray = @[
            ...
            @"in:secure:signal:jingle:1",
            @"in:secure:signal:jingle:apps:rtp:1",
            @"in:secure:signal:jingle:apps:rtp:audio",
            @"in:secure:signal:jingle:transports:raw-udp:0",
            @"in:secure:signal:jingle:transports:raw-udp:1",
            @"jabber:x:oob",
            @"in:secure:signal:ping",
            @"in:secure:signal:receipts",
            @"in:secure:signal:idle:1",
            @"http://jabber.org/protocol/chatstates",
            @"jabber:iq:version",
            @"in:secure:signal:chat-markers:0"
        ];
