...
-(void) handleXMPPError:(NSNotification*) notification
{
    ...
    if([notification.userInfo[@"isSevere"] boolValue])
    {
        ...
        if ([notification.userInfo[@"message"] isEqualToString:@"Security Check"]){
            return;
        }
        //NSString* idval = xmppAccount.connectionProperties.identity.user;        //use this to only show the newest error notification per account
//        UNMutableNotificationContent* content = [[UNMutableNotificationContent alloc] init];
//        content.title = xmppAccount.connectionProperties.identity.user;
...
-(void) handleNewMessage:(NSNotification*) notification
{
    ...
    xmpp* xmppAccount = notification.object;
    [[MLNotificationQueue currentQueue] postNotificationName:kMonalStoreMessageNotice object:xmppAccount userInfo:nil];
...
-(void) showModernNotificaionForMessage:(MLMessage*) message withSound:(BOOL) sound
{
    ...
    if(self.notificationPrivacySetting <= DisplayOnlyName)
    {
        ...
        if(message.isMuc){
            if ([message.participantJid containsString:@"@conference.chat.securesignal.in@chat.securesignal.in"]){
                               NSArray *components = [message.participantJid componentsSeparatedByString:@"@conference.chat.securesignal.in@chat.securesignal.in"];
                        content.subtitle = [NSString stringWithFormat:NSLocalizedString(@"%@ says:", @""),components[0]];
            }else if ([message.participantJid containsString:@"@chat.securesignal.in"]){
                NSArray *components = [message.participantJid componentsSeparatedByString:@"@chat.securesignal.in"];
         content.subtitle = [NSString stringWithFormat:NSLocalizedString(@"%@ says:", @""),components[0]];
            }else{
                               
                               content.subtitle = [NSString stringWithFormat:NSLocalizedString(@"%@ says:", @""), message.participantJid];
                           }
        }
    ...
    if(self.notificationPrivacySetting == DisplayNameAndMessage)
    {
        ...
        if([message.messageType isEqualToString:kMessageTypeFiletransfer])
        {
            ...
            content.body = NSLocalizedString(@"Replied to message", @"");
        else if([message.messageType isEqualToString:KMessageDecrypt])
            content.body = NSLocalizedString(@"Sent a Message 🔒", @"");
        ...
        else if ([message.messageText containsString:kMessageTypeCreateGroup])
            content.body = NSLocalizedString(@"Created Group", @"");
        else if ([message.messageText containsString:kMessageTypeAddMember])
            content.body = NSLocalizedString(@"Added Member", @"");
        else if ([message.messageText containsString:kMessageTypeChangedGroupSubjectName])
            content.body = NSLocalizedString(@"Changed Group Subject", @"");
        else if ([message.messageText containsString:kMessageTypeChangedGroupInfo])
            content.body = NSLocalizedString( @"Changed Group Info", @"");
    }
    else
