...
+(void) checkMimeTypeAndSizeForHistoryID:(NSNumber*) historyId
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSString* link;
        if ([msg.messageText containsString:kMessageTypeImageCaption]){
            NSArray *msgcomponents = [msg.messageText componentsSeparatedByString:kMessageTypeImageCaption];
            link = msgcomponents[0];
            msg.messageType = kMessageTypeImageCaption;
        }else{
            link = msg.messageText;
        }
        NSString* url = [self genCanonicalUrl:link];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSURLSessionDownloadTask* task = [session downloadTaskWithURL:[NSURL URLWithString:url] completionHandler:^(NSURL* _Nullable location, NSURLResponse* _Nullable response, NSError* _Nullable error) {
            ...
            if([[urlComponents.scheme lowercaseString] isEqualToString:@"aesgcm"])
            {
                ...
                if (urlComponents.fragment.length == 96 ){
                    int ivLength = 32;
                                 ...
                                 if(encryptedData && encryptedData.length > 0 && key && key.length == 32 && iv && iv.length == 16)
                                 {
                 //
                                     NSData* decryptedData = [AESGcm AESdecrypt:encryptedData withKey:key andIv:iv withAuth:nil];
                ...
                }else if (urlComponents.fragment.length == 88){
                    int ivLength = 24;
                                                    //format is iv+32byte key
                                    NSData* key = [HelperTools dataWithHexString:[urlComponents.fragment substringWithRange:NSMakeRange(ivLength, 64)]];
                                    NSData* iv = [HelperTools dataWithHexString:[urlComponents.fragment substringToIndex:ivLength]];
