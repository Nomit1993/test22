...
-(void) connectAccountWithDictionary:(NSDictionary*) account
{
    ...
    if(existing)
    {
        ...
        if(_hasConnectivity)
            [existing sendPing:SHORT_PING];
        //short ping timeout to quickly check if connectivity is still okay
        else
...
-(void) sendGroupMessage:(NSString*) message toContact:(MLContact*) contact isEncrypted:(BOOL) encrypted isUpload:(BOOL) isUpload messageId:(NSString*) messageId withCompletionHandler:(void (^)(BOOL success, NSString* messageId)) completion
{
    BOOL success = NO;
    xmpp* account = [self getConnectedAccountForID:contact.accountId];
    if(account)
...
-(NSString*) getAccountNameForConnectedRow:(NSInteger) row
{
    ...
    if(account)
        return account.connectionProperties.identity.user;
    return @"";
}
