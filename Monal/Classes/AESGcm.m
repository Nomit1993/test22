...
+(NSData*) AESdecrypt:(NSData*) body withKey:(NSData*) key andIv:(NSData*) iv withAuth:(NSData* _Nullable) auth
{
    MLCrypto* crypto = [[MLCrypto alloc] init];
    
    NSData* toReturn = [crypto decryptAESGCMWithKey:key encryptedContent:body iv:iv authentication:auth];
    return toReturn;
}
