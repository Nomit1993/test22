...
-(id) initWithServer:(nonnull MLXMPPServer *) server andIdentity:(nonnull MLXMPPIdentity *)identity andAccountNo:(NSString*) accountNo
{
    ...
    [self.pubsub registerForNode:@"in:secure:signal:avatar:metadata" withHandler:$newHandler(MLPubSubProcessor, avatarHandler)];
...
-(void)fetchBuddiesAvatar:(NSString *)contactJid{
    XMPPIQ* vcardNode = [[XMPPIQ alloc] initWithType:kiqGetType];
    ...
    [vcardNode setiqFrom:FromJid];
    [vcardNode setiqTo:contactJid];
    MLXMLNode* queryNode = [[MLXMLNode alloc] init];
    queryNode.element = @"vCard";
    [queryNode.attributes setObject:@"vcard-temp" forKey:kXMLNS];
    [vcardNode addChild:queryNode];
    xmpp* account = [[MLXMPPManager sharedInstance].connectedXMPP objectAtIndex:0];
    [account sendIq:vcardNode withResponseHandler:^(XMPPIQ * successResult) {
        DDLogInfo(@"vCard success ping returned: everything is fine");
        if ([successResult check:@"/<type=result>"])
        {
           NSString *binvalData = [successResult findFirst:@"{vcard-temp}vCard/PHOTO/BINVAL#"];
            DDLogInfo(@"picture %@:",binvalData);
            if (binvalData != nil) {
                NSData * vcardData = [HelperTools dataWithBase64EncodedString:binvalData];
                [[MLImageManager sharedInstance] setIconForContact:contactJid andAccount:account.accountNo WithData:vcardData];
            }
         
        }
    } andErrorHandler:^(XMPPIQ * error) {
        DDLogInfo(@"vcard fetch failure ping returned: everything is fine");
    }];
}
    ...
    [account sendIq:vcardNode withResponseHandler:^(XMPPIQ * successResult) {
        ...
        if ([successResult check:@"/<type=result>"])
        {
           NSString *binvalData = [successResult findFirst:@"{vcard-temp}vCard/PHOTO/BINVAL#"];
            ...
            if (binvalData != nil){
                NSData * vcardData = [HelperTools dataWithBase64EncodedString:binvalData];
                [[MLImageManager sharedInstance] setIconForContact:FromJid andAccount:account.accountNo WithData:vcardData];
            }
...
-(void) setupObjects
{
    ...
    _capsIdentity = [[MLXMLNode alloc] initWithElement:@"identity" withAttributes:@{
        @"category": @"client",
        @"type": @"phone",
        @"name": [NSString stringWithFormat:@"SAI %@", [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleShortVersionString"]]
...
-(BOOL) connectionTask
{
    ...
    if(!_SRVDiscoveryDone)
    {
        ...
        if([_discoveredServersList count]==0)
        {
            ...
            [self.connectionProperties.server updateConnectPort:@52202];
...
-(void) startXMPPStream:(BOOL) clearReceiveQueue
{
    ...
    if(!_baseParserDelegate)
    {
        ...
        _baseParserDelegate = [[MLBasePaser alloc] initWithCompletion:^(MLXMLNode* _Nullable parsedStanza) {
            ...
            if([self->_parseQueue operationCount] > 1)
            {
                ...
                [parsedStanza find:@"/{in:secure:signal:sm:3}r"];
                [parsedStanza find:@"/{in:secure:signal:sm:3}a"];
                [parsedStanza find:@"/<type=get>"];
                [parsedStanza find:@"/<type=set>"];
                [parsedStanza find:@"/<type=result>"];
                [parsedStanza find:@"/<type=error>"];
                [parsedStanza find:@"{in:secure:signal:sid:0}origin-id"];
                [parsedStanza find:@"/{c5b9cdd82abcf6305f9c24fa5b7715e15dfe36fa810852494dad0297fd9dc866:client}presence"];
                [parsedStanza find:@"/{c5b9cdd82abcf6305f9c24fa5b7715e15dfe36fa810852494dad0297fd9dc866:client}message"];
                [parsedStanza find:@"/@h|int"];
                [parsedStanza find:@"{in:secure:signal:delay}delay"];
                ...
                [parsedStanza find:@"{in:secure:signal:receipts}received@id"];
                [parsedStanza find:@"{http://jabber.org/protocol/chatstates}*"];
                [parsedStanza find:@"{in.securesignal.secure.service}encrypted/payload"];
                [parsedStanza find:@"{in:secure:signal:sid:0}stanza-id@by"];
                [parsedStanza find:@"{in:secure:signal:mam:2}result"];
                [parsedStanza find:@"{in:secure:signal:chat-markers:0}displayed@id"];
                [parsedStanza find:@"body"];
                [parsedStanza find:@"{in:secure:signal:mam:2}result@id"];
                [parsedStanza find:@"{in:secure:signal:carbons:2}*"];
            }
#endif
    ...
    MLXMLNode* stream = [[MLXMLNode alloc] initWithElement:@"stream:stream" andNamespace:@"c5b9cdd82abcf6305f9c24fa5b7715e15dfe36fa810852494dad0297fd9dc866:client"];
...
-(void) requestSMAck:(BOOL) force
{
    ...
    @synchronized(_stateLockObject) {
        ...
        if(self.accountState>=kStateBound && self.connectionProperties.supportsSM3 &&
            ...
            rNode = [[MLXMLNode alloc] initWithElement:@"r" andNamespace:@"in:secure:signal:sm:3" withAttributes:dic andChildren:@[] andData:nil];
...
-(void) sendSMAck:(BOOL) queuedSend
{
    ...
    MLXMLNode* aNode = [[MLXMLNode alloc] initWithElement:@"a" andNamespace:@"in:secure:signal:sm:3" withAttributes:dic andChildren:@[] andData:nil];
    if(queuedSend)
        [self send:aNode];
...
-(void) processInput:(MLXMLNode*) parsedStanza
{
    ...
    {
        if([parsedStanza check:@"/{in:secure:signal:sm:3}r"] && self.connectionProperties.supportsSM3 && self.accountState>=kStateBound)
        {
            [self sendSMAck:YES];
        }
        else if([parsedStanza check:@"/{in:secure:signal:sm:3}a"] && self.connectionProperties.supportsSM3 && self.accountState>=kStateBound)
        {
        ...
        else if([parsedStanza check:@"/{c5b9cdd82abcf6305f9c24fa5b7715e15dfe36fa810852494dad0297fd9dc866:client}presence"])
        {
            ...
            else
            {
                ...
                if(![presenceNode check:@"/@type"])
                {
                    ...
                    if(presenceNode.from)
                    {
                        ...
                        if([presenceNode check:@"{in:secure:signal:idle:1}idle@since"])
                        {
                            NSDate* lastInteraction = [[DataLayer sharedInstance] lastInteractionOfJid:presenceNode.fromUser forAccountNo:self.accountNo];
                            if([[presenceNode findFirst:@"{in:secure:signal:idle:1}idle@since|datetime"] compare:lastInteraction] == NSOrderedDescending)
                            {
                                [[DataLayer sharedInstance] setLastInteraction:[presenceNode findFirst:@"{in:secure:signal:idle:1}idle@since|datetime"] forJid:presenceNode.fromUser andAccountNo:self.accountNo];
                                ...
                                [[MLNotificationQueue currentQueue] postNotificationName:kMonalLastInteractionUpdatedNotice object:self userInfo:@{
                                    ...
                                    @"lastInteraction": [presenceNode findFirst:@"{in:secure:signal:idle:1}idle@since|datetime"],
                                    @"isTyping": @NO
                                }];
                            }
                        }
        ...
        else if([parsedStanza check:@"/{c5b9cdd82abcf6305f9c24fa5b7715e15dfe36fa810852494dad0297fd9dc866:client}message"])
        {
            ...
            if([outerMessageNode check:@"{in:secure:signal:mam:2}result"])          //mam result
            {
                ...
                @synchronized(_stateLockObject) {
                    if(_runningMamQueries[[outerMessageNode findFirst:@"{in:secure:signal:mam:2}result@queryid"]] == nil)
                    {
                        DDLogError(@"mam results must be asked for, ignoring this spoofed mam result having queryid: %@!", [outerMessageNode findFirst:@"{in:secure:signal:mam:2}result@queryid"]);
                ...
                messageNode = [[XMPPMessage alloc] initWithXMPPMessage:[outerMessageNode findFirst:@"{in:secure:signal:mam:2}result/{in:secure:signal:forward:0}forwarded/{c5b9cdd82abcf6305f9c24fa5b7715e15dfe36fa810852494dad0297fd9dc866:client}message"]];
                ...
                if([outerMessageNode check:@"{in:secure:signal:mam:2}result/{in:secure:signal:forward:0}forwarded/{in:secure:signal:delay}delay"] && ![messageNode check:@"{in:secure:signal:delay}delay"])
                    [messageNode addChild:[outerMessageNode findFirst:@"{in:secure:signal:mam:2}result/{in:secure:signal:forward:0}forwarded/{in:secure:signal:delay}delay"]];
                
                DDLogDebug(@"mam extracted, messageNode is now: %@", messageNode);
            }
            else if(self.connectionProperties.usingCarbons2 && [outerMessageNode check:@"{in:secure:signal:carbons:2}*"])     //carbon copy
            {
                ...
                //create a new XMPPMessage node instead of only a MLXMLNode because messages have some convenience properties and methods
                messageNode = [[XMPPMessage alloc] initWithXMPPMessage:[outerMessageNode findFirst:@"{in:secure:signal:carbons:2}*/{in:secure:signal:forward:0}forwarded/{c5b9cdd82abcf6305f9c24fa5b7715e15dfe36fa810852494dad0297fd9dc866:client}message"]];
                
                //move carbon copy delay timestamp into forwarded message stanza if the forwarded stanza does not have one already
                //that makes parsing a lot easier later on and should not do any harm, even when resending/forwarding this inner stanza
                if([outerMessageNode check:@"{in:secure:signal:delay}delay"] && ![messageNode check:@"{in:secure:signal:delay}delay"])
                    [messageNode addChild:[outerMessageNode findFirst:@"{in:secure:signal:delay}delay"]];
            ...
            if(![outerMessageNode check:@"{in:secure:signal:mam:2}result"] || [[outerMessageNode findFirst:@"{in:secure:signal:mam:2}result@queryid"] hasPrefix:@"MLcatchup:"])
            {
                //process message
                ...
                if(!stanzaid)
                {
                    if(![messageNode check:@"/<type=groupchat>"] && [self.connectionProperties.identity.jid isEqualToString:[messageNode findFirst:@"{in:secure:signal:sid:0}stanza-id@by"]])
            ...
            else if([[outerMessageNode findFirst:@"{in:secure:signal:mam:2}result@queryid"] hasPrefix:@"MLhistory:"])
        ...
        else if([parsedStanza check:@"/{c5b9cdd82abcf6305f9c24fa5b7715e15dfe36fa810852494dad0297fd9dc866:client}iq"])
        {
            ...
            if([iqNode check:@"/<type=result>/{in:secure:signal:mam:2}fin"] && _runningMamQueries[[iqNode findFirst:@"/@id"]] != nil)
        ...
        else if([parsedStanza check:@"/{in:secure:signal:sm:3}enabled"])
        {
        ...
        else if([parsedStanza check:@"/{in:secure:signal:sm:3}resumed"] && self.connectionProperties.supportsSM3 && self.accountState<kStateBound)
        {
            NSNumber* h = [parsedStanza findFirst:@"/@h|int"];
            if(h==nil)
                return [self invalidXMLError];
        ...
        else if([parsedStanza check:@"/{in:secure:signal:sm:3}failed"] && self.connectionProperties.supportsSM3 && self.accountState<kStateBound && self.resuming)
        {
        ...
        else if([parsedStanza check:@"/{in:secure:signal:xml:ns:xmpp-sasl}failure"])
        {
        ...
        else if([parsedStanza check:@"/{in:secure:signal:xml:ns:xmpp-sasl}challenge"])
        {
            if(self.accountState >= kStateLoggedIn)
                return [self invalidXMLError];
            if(self.connectionProperties.server.isDirectTLS || self->_startTLSComplete)
            {
                MLXMLNode* responseXML = [[MLXMLNode alloc] initWithElement:@"response" andNamespace:@"in:secure:signal:xml:ns:xmpp-sasl"];
        ...
        else if([parsedStanza check:@"/{in:secure:signal:xml:ns:xmpp-sasl}success"])
        {
            if(self.accountState >= kStateLoggedIn)
                return [self invalidXMLError];
        ...
        {
            NSString* errorReason = [parsedStanza findFirst:@"{in:secure:signal:xml:ns:xmpp-streams}!text$"];
            NSString* errorText = [parsedStanza findFirst:@"{in:secure:signal:xml:ns:xmpp-streams}text#"];
        ...
        else if([parsedStanza check:@"/{http://etherx.jabber.org/streams}features"])
        {
            ...
            if(self.accountState < kStateLoggedIn)
            {
                ...
                else
                {
                    ...
                    NSSet* supportedSaslMechanisms = [NSSet setWithArray:[parsedStanza find:@"{in:secure:signal:xml:ns:xmpp-sasl}mechanisms/mechanism#"]];
                    
                    if([supportedSaslMechanisms containsObject:@"PLAIN"])
                    {
                        [self send:[[MLXMLNode alloc]
                            initWithElement:@"auth"
                            andNamespace:@"in:secure:signal:xml:ns:xmpp-sasl"
            ...
            else
            {
                ...
                @synchronized(_stateLockObject) {
                    ...
                    if(self.connectionProperties.supportsSM3 && self.streamID)
                    {
                        ...
                        resumeNode = [[MLXMLNode alloc] initWithElement:@"resume" andNamespace:@"in:secure:signal:sm:3" withAttributes:dic andChildren:@[] andData:nil];
    ...
    else
    {
        ...
        else if([parsedStanza check:@"/{http://etherx.jabber.org/streams}features"])
        {
            ...
            MLXMLNode* startTLS = [[MLXMLNode alloc] initWithElement:@"startenc" andNamespace:@"in:secure:signal:xml:ns:xmpp-tls"];
            [self send:startTLS];
            return;
        }
        else if([parsedStanza check:@"/{in:secure:signal:xml:ns:xmpp-tls}proceed"])
        {
...
-(void) send:(MLXMLNode*) stanza withSmacks:(BOOL) withSmacks
{
    ...
    [self dispatchAsyncOnReceiveQueue:^{
        ...
        NSString* mamQueryId = [stanza findFirst:@"/{c5b9cdd82abcf6305f9c24fa5b7715e15dfe36fa810852494dad0297fd9dc866:client}iq/{in:secure:signal:mam:2}query@queryid"];
        if(mamQueryId)
            @synchronized(self->_stateLockObject) {
        ...
        if(withSmacks && [stanza isKindOfClass:[XMPPStanza class]])
        {
            ...
            if(![queued_stanza.element isEqualToString:@"iq"])      //add delay tag to message or presence stanzas but not to iq stanzas
            {
                ...
                if(![queued_stanza check:@"{in:secure:signal:delay}delay"])
        ...
        BOOL isBindRequest = [stanza isKindOfClass:[XMPPIQ class]] && [stanza check:@"{in:secure:signal:xml:ns:xmpp-bind}bind/resource"];
        ...
        )
        {
            [self->_sendQueue addOperation:[NSBlockOperation blockOperationWithBlock:^{
                if([stanza check:@"/{in:secure:signal:xml:ns:xmpp-sasl}*"])
                    DDLogDebug(@"SEND: redacted sasl element: %@", [stanza findFirst:@"/{in:secure:signal:xml:ns:xmpp-sasl}*$"]);
...
-(void) sendGroupMessage:(NSString*) message toContact:(MLContact*) contact isEncrypted:(BOOL) encrypt isUpload:(BOOL) isUpload andMessageId:(NSString*) messageId{
    
    [self sendGroupMessage:message toContact:contact isEncrypted:encrypt isUpload:isUpload andMessageId:messageId withLMCId:nil];
}
    
-(void) sendGroupMessage:(NSString*) message toContact:(MLContact*) contact isEncrypted:(BOOL) encrypt isUpload:(BOOL) isUpload andMessageId:(NSString*) messageId withLMCId:(NSString* _Nullable) LMCId
{
    XMPPMessage* messageNode = [[XMPPMessage alloc] init];
    NSArray* accountList = [[DataLayer sharedInstance] accountList];
    NSString *FromJid = [NSString stringWithFormat:@"%@@%@", [[accountList objectAtIndex:0] objectForKey:@"username"],[[accountList objectAtIndex:0] objectForKey:@"domain"]];
    
    [messageNode.attributes setObject:FromJid forKey:@"from"];
...
//    
    if (!contact.isGroup){
        ...
        NSMutableArray<MLMessage*>* messages = [[DataLayer sharedInstance] messagesForContact:contact.contactJid forAccount:contact.accountId];
        
        [self.omemo queryOMEMODevices:contact.contactJid];
        devices = [[NSMutableArray alloc] initWithArray:[self.omemo knownDevicesForAddressName:contact.contactJid]];
        for(NSNumber* device in devices) {
            ...
            NSData* identity = [self.omemo.monalSignalStore getIdentityForAddress:address];
            BOOL newTrust;
            int internalTrustLevel = [self.omemo.monalSignalStore getInternalTrustLevel:address identityKey:identity];
            if(internalTrustLevel == MLOmemoInternalTrusted) {
                newTrust = NO;
            } else { // MLOmemoInternalToFU || MLOmemoInternalNotTrusted
                newTrust = YES;
                [self.omemo updateTrust:newTrust forAddress:address];
            }
            if ([messages count] == 1){
                MLSignalStore *  monalSignalStore = [[MLSignalStore alloc] initWithAccountId:self.accountNo];
                SignalAddress* address = [[SignalAddress alloc] initWithName:contact.contactJid deviceId:(uint32_t)device.intValue];

                NSData* identity = [monalSignalStore getIdentityForAddress:address];

    //            // Only add encryption key for devices that are trusted
                if([monalSignalStore isTrustedIdentity:address identityKey:identity])
                {
                [self.omemo needNewSessionForContact:contact.contactJid andDevice:device];
                }
            }
    ...
    if(encrypt && !contact.isGroup)
            if ([message containsString:@"request_to_save_muc_key"] || [message containsString:@"request_for_muc_key"]){
                [self.omemo encryptGroupMessage:messageNode withMessage:message toContact:contact.contactJid];
            }else{
                [self.omemo encryptMessage:messageNode withMessage:message toContact:contact.contactJid];
            }
...
//    if ([contact.contactJid containsString:@"enhanced-apk@chat.securesignal.in"]){
//        NSString *groupSubject = [[HelperTools defaultsDB] stringForKey:@"Subject_Group"];
//        [messageNode setSubjectBody:groupSubject];
//    }
    ...
    {
        [messageNode addChild:[[MLXMLNode alloc] initWithElement:@"request" andNamespace:@"in:secure:signal:receipts"]];
        [messageNode addChild:[[MLXMLNode alloc] initWithElement:@"markable" andNamespace:@"in:secure:signal:chat-markers:0"]];
    }
    ...
    if ([message containsString:@"request_to_save_muc_key"] || [message containsString:@"request_for_muc_key"]){
        MLXMLNode* key_id = [[MLXMLNode alloc] initWithElement:@"muc-request-key"];
        [key_id.attributes setObject:@"in.securesignal.secure.service" forKey:kXMLNS];
        
        [key_id.attributes setObject:message forKey:@"muc-key-id"];
        [messageNode addChild:key_id];
    }
    //handle LMC
    if(LMCId)
    ...
    if (contact.isGroup){
          //  [self checkMUCSubscribe:contact.contactJid completion:^(BOOL status) {
            //            if (status == YES ){
                            xmpp *account = [[MLXMPPManager sharedInstance] getConnectedAccountForID:contact.accountId];
                            [MLMucProcessor moderatorMsgSubscriberoom:contact.contactJid onAccount:account];
                            XMPPMessage* messageGroupNode = [[XMPPMessage alloc] init];
                            NSArray* accountList = [[DataLayer sharedInstance] accountList];
                            NSString *FromJid = [NSString stringWithFormat:@"%@@%@", [[accountList objectAtIndex:0] objectForKey:@"username"],[[accountList objectAtIndex:0] objectForKey:@"domain"]];
                            [messageGroupNode.attributes setObject:FromJid forKey:@"from"];
                            [messageGroupNode.attributes setObject:contact.contactJid forKey:@"to"];
                            [messageGroupNode.attributes setObject:kMessageGroupChatType forKey:@"type"];
                            messageGroupNode.id = messageId;
                            [messageGroupNode setBody:message];
                            [self send:messageGroupNode];
...
//    for(NSDictionary* entry in [[DataLayer sharedInstance] listMucsForAccount:self.accountNo]){
//          [MLMucProcessor join:entry[@"room"] onAccount:self];
//          [MLMucProcessor fetchMembersList:entry[@"room"] onAccount:self];
//          [MLMucProcessor moderatorSubscriberoom:entry[@"room"] onAccount:self];
//          [MLMucProcessor moderatorMsgSubscriberoom:entry[@"room" ] onAccount:self];
//    }
...
-(void) requestHTTPSlotWithParams:(NSDictionary*) params andCompletion:(void(^)(NSString* url, NSError* error)) completion
{
    ...
    [self sendIq:httpSlotRequest withResponseHandler:^(XMPPIQ* response) {
        DDLogInfo(@"Got slot for upload: %@", [response findFirst:@"{in:secure:signal:http:upload:0}slot/put@url"]);
        //upload to server using HTTP PUT
        NSMutableDictionary* headers = [[NSMutableDictionary alloc] init];
        headers[@"Content-Type"] = [params objectForKey:kContentType];
        for(MLXMLNode* header in [response find:@"{in:secure:signal:http:upload:0}slot/put/header"])
            headers[[header findFirst:@"/@name"]] = [header findFirst:@"/#"];
        dispatch_async(dispatch_get_main_queue(), ^{
            [MLHTTPRequest
                sendWithVerb:kPut path:[response findFirst:@"{in:secure:signal:http:upload:0}slot/put@url"]
                headers:headers
                withArguments:nil
                ...
                andCompletionHandler:^(NSError* error, id result) {
                    if(!error)
                    {
                        DDLogInfo(@"Upload succeded, get url: %@", [response findFirst:@"{in:secure:signal:http:upload:0}slot/get@url"]);
                        //send get url to contact
                        if(completion)
                            completion([response findFirst:@"{in:secure:signal:http:upload:0}slot/get@url"], nil);
                    }
                    else
                    {
...
-(void) sendCurrentCSIState
{
    [self dispatchOnReceiveQueue: ^{
        ...
        MLXMLNode* csiNode;
        if(self->_isCSIActive)
            csiNode = [[MLXMLNode alloc] initWithElement:@"active" andNamespace:@"in:secure:signal:csi:0"];
        else
            csiNode = [[MLXMLNode alloc] initWithElement:@"inactive" andNamespace:@"in:secure:signal:csi:0"];
        [self send:csiNode];
    }];
}
...
-(void) setMAMQueryMostRecentForContact:(MLContact*) contact before:(NSString*) uid withCompletion:(void (^)(NSArray* _Nullable, NSString* _Nullable error)) completion
{
    ...
    responseHandler = ^(XMPPIQ* response) {
        ...
        else
        {
            ...
            if(
                ![[response findFirst:@"{in:secure:signal:mam:2}fin@complete|bool"] boolValue] &&
                [response check:@"{in:secure:signal:mam:2}fin/{http://jabber.org/protocol/rsm}set/first#"]
            )
            {
                query([response findFirst:@"{in:secure:signal:mam:2}fin/{http://jabber.org/protocol/rsm}set/first#"]);
            }
            else
            {
    ...
    query = ^(NSString* _Nullable before) {
        ...
        [self sendIq:query withResponseHandler:responseHandler andErrorHandler:^(XMPPIQ* error) {
            ...
            if(retrievedBodies == 0)
            {
                ...
                if(error == nil || ([error check:@"error/{in:secure:signal:xml:ns:xmpp-stanzas}internal-server-error"] && [@"item-not-found" isEqualToString:[error findFirst:@"error/{in:secure:signal:xml:ns:xmpp-stanzas}text#"]]))
                    completion(nil, nil);
                else
...
-(void) createMuc:(NSString* _Nonnull)room subject:(NSString* _Nonnull) roomSubject completion:(void (^)(BOOL status))completion
{
    [MLMucProcessor create:room onAccount:self subject:roomSubject completion:^(BOOL success) {
        if (success == YES){
            completion(YES);
        }else{
            completion(NO);
   ...
   // [MLMucProcessor create:room onAccount:self subject:roomSubject];
}

-(void) grantRoleToMUC:(NSString * _Nonnull) jid room:(NSString * _Nonnull)roomJid{
    [MLMucProcessor grantRole:jid room:roomJid onAccount:self];
}
-(void) grantMemberToMUC:(NSString * _Nonnull) jid room:(NSString * _Nonnull)roomJid{
    [MLMucProcessor grantMembership:jid room:roomJid onAccount:self];
}
-(void) invitetoMUC:(NSString * _Nonnull) jid room:(NSString * _Nonnull)roomJid {
    [MLMucProcessor inviteUser:jid room:roomJid onAccount:self];
}
-(void) changeMUCSubject:(NSString * _Nonnull)jid roomSubject:(NSString * _Nonnull)subject{
    
    [MLMucProcessor changeMucSubject:subject room:jid onAccount:self];
}

-(void) subscribetoMUC:(NSString * _Nonnull) jid room:(NSString * _Nonnull)roomJid{
    [MLMucProcessor mucSubscribeUser:jid room:roomJid onAccount:self];
}
-(void) checkMUCSubscribe:(NSString *)roomJID completion:(void (^)(BOOL status))completion{
    [MLMucProcessor checkSubscriberoom:roomJID onAccount:self completion:^(BOOL success) {
            if (success == YES){
                completion(YES);
            }else{
                completion(NO);
...
-(void) moderatorMsgSubscriberoom:(NSString *)roomJid{
    [MLMucProcessor moderatorMsgSubscriberoom:roomJid onAccount:self];
}
-(void) moderatorSubscriptiontoRoom:(NSString *_Nonnull)roomJid {
    [MLMucProcessor moderatorSubscriberoom:roomJid onAccount:self];
}
...
-(void) checkJidType:(NSString*) jid withCompletion:(void (^)(NSString* type, NSString* _Nullable errorMessage)) completion
{
    ...
    [self sendIq:discoInfo withResponseHandler:^(XMPPIQ* response) {
        ...
        if([error check:@"/<type=error>/error<type=cancel>/{in:secure:signal:xml:ns:xmpp-stanzas}service-unavailable"])
            return completion(@"account", nil);
        else if([error check:@"/<type=error>/error<type=auth>/{in:secure:signal:xml:ns:xmpp-stanzas}subscription-required"])
            return completion(@"account", nil);
...
//        NSString *jid = [NSString stringWithFormat:@"%@@%@",[[accountList objectAtIndex:0] objectForKey:@"username"],[[accountList objectAtIndex:0] objectForKey:@"domain"]];
       // [registerNode setiqTo:jid];//resource
        //[self sendIq:registerNode withHandler:$newHandler(MLIQProcessor, handleAppserverNodeRegistered)];
//        PushServices *apnsService = [[PushServices alloc]init];
//        [apnsService registerPushserviceWithJid:jid username:[[accountList objectAtIndex:0] objectForKey:@"username"] pushToken:pushToken completion:^(BOOL status, NSString * _Nullable error) {
//                   if (status == YES && error == nil){
//                       DDLogInfo(@"registering of the push service success");
//                   }else{
//                       DDLogInfo(@"registering of the push service failed");
//                       //[self enablePush];
//                   }
//        }];
...
-(void) addMessageToMamPageArray:(NSDictionary*) messageDictionary
{
    @synchronized(_mamPageArrays) {
        if(!_mamPageArrays[[messageDictionary[@"outerMessageNode"] findFirst:@"{in:secure:signal:mam:2}result@queryid"]])
            _mamPageArrays[[messageDictionary[@"outerMessageNode"] findFirst:@"{in:secure:signal:mam:2}result@queryid"]] = [[NSMutableArray alloc] init];
        [_mamPageArrays[[messageDictionary[@"outerMessageNode"] findFirst:@"{in:secure:signal:mam:2}result@queryid"]] addObject:messageDictionary];
    }
}
...
-(void) publishGroupAvatar:(UIImage*) image groupJid:(NSString*)contactJid
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if(!image)
        {
            DDLogInfo(@"Retracting own avatar image");
            [self.pubsub deleteNode:@"in:secure:signal:avatar:metadata" andHandler:$newHandler(MLPubSubProcessor, avatarDeleted)];
            [self.pubsub deleteNode:@"in:secure:signal:avatar:data" andHandler:$newHandler(MLPubSubProcessor, avatarDeleted)];
        }
        else
        {
...
//            NSString* imageHash = [HelperTools hexadecimalString:[HelperTools sha1:data]];
//
//            DDLogInfo(@"Publishing own avatar image with hash %@", imageHash);
            
            XMPPIQ* vcardNode = [[XMPPIQ alloc] initWithType:kiqSetType];
            NSArray* accountList = [[DataLayer sharedInstance] accountList];
            NSString *FromJid = [NSString stringWithFormat:@"%@@%@", [[accountList objectAtIndex:0] objectForKey:@"username"],[[accountList objectAtIndex:0] objectForKey:@"domain"]];
            [vcardNode setiqFrom:FromJid];
            [vcardNode setiqTo:contactJid];
            ...
            queryNode.element = @"vCard";
            [queryNode.attributes setObject:@"vcard-temp" forKey:kXMLNS];
            MLXMLNode* photoNode = [[MLXMLNode alloc] init];
            photoNode.element = @"PHOTO";
            MLXMLNode* typeNode = [[MLXMLNode alloc] init];
            typeNode.element = @"TYPE";
            typeNode.data = @"image/jpeg";
            MLXMLNode* binvalNode = [[MLXMLNode alloc] init];
            binvalNode.element = @"BINVAL";
            binvalNode.data = [HelperTools encodeBase64WithData:data];
            
            [photoNode addChild:typeNode];
            [photoNode addChild:binvalNode];
            [queryNode addChild:photoNode];
            [vcardNode addChild:queryNode];
            ...
            [account sendIq:vcardNode withResponseHandler:^(XMPPIQ * successResult) {
                ...
                if ([successResult check:@"/<type=result>"])
                {
                    [self fetchBuddiesAvatar:contactJid];
                    DDLogInfo(@"Avatar success ping returned: everything is fine");
                }
            } andErrorHandler:^(XMPPIQ * error) {
...
//                [[MLXMLNode alloc] initWithElement:@"data" andNamespace:@"in:secure:signal:avatar:data" withAttributes:@{} andChildren:@[] andData:[HelperTools encodeBase64WithData:data]]
//            ] andData:nil];
//            NSArray* accountList = [[DataLayer sharedInstance] accountList];
//            NSString *FromJid = [NSString stringWithFormat:@"%@@%@/%@", [[accountList objectAtIndex:0] objectForKey:@"username"],[[accountList objectAtIndex:0] objectForKey:@"domain"],[[accountList objectAtIndex:0] objectForKey:@"resource"]];
//            XMPPIQ* query = [[XMPPIQ alloc] initWithType:kiqSetType];
//            [query setiqFrom:FromJid];
...
//                [[MLXMLNode alloc] initWithElement:@"publish" withAttributes:@{@"node": @"in:secure:signal:avatar:data"} andChildren:@[item] andData:nil]
//            ] andData:nil]];
            
           
//            [account sendIq:query withResponseHandler:^(XMPPIQ * successResult) {
//                DDLogInfo(@"Avatar success ping returned: everything is fine");
//
//                if ([successResult check:@"/<type=result>"])
//                {
...
//
//                    }];
//            [self.pubsub publishItem:item onNode:@"in:secure:signal:avatar:data" withConfigOptions:@{
//                @"pubsub#persist_items": @"true",
//                @"pubsub#access_model": @"presence"
