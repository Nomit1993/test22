...
-(UIImage *) inboundImage
{
 if (_inboundImage)
 {
    ...
    _inboundImage=[[UIImage imageNamed:@"outgoing"]
...
//    }
    //UIColor *color = [UIColor monalGreen];
    _outboundImage = [[UIImage imageNamed:@"incoming"]
                   resizableImageWithCapInsets:UIEdgeInsetsMake(6, 6, 6, 6)];
      
//        _outboundImage = [[UIImage imageNamed:@"outgoing"]
//                          resizableImageWithCapInsets:UIEdgeInsetsMake(6, 6, 6, 6)];// Image to mask with
//        UIGraphicsBeginImageContextWithOptions(_outboundImage.size, NO, _outboundImage.scale);
//        CGContextRef context = UIGraphicsGetCurrentContext();
//        [color setFill];
//        CGContextTranslateCTM(context, 0, _outboundImage.size.height);
//        CGContextScaleCTM(context, 1.0, -1.0);
//        CGContextClipToMask(context, CGRectMake(0, 0, _outboundImage.size.width, _outboundImage.size.height), [_outboundImage CGImage]);
//        CGContextFillRect(context, CGRectMake(0, 0, _outboundImage.size.width, _outboundImage.size.height));
//
...
//        }
        ...
        if ([contact containsString:@"conference.chat.securesignal.in"] && toreturn == nil){
        
       
            toreturn = nil;
            //[UIImage imageNamed:@"noicon_muc"];
                  
                   
               }else if (toreturn == nil)
               {
                   toreturn = nil;
                  // self.noIcon;
               }
