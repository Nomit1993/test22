...
- (void)viewDidLoad
{
    ...
    if(self.originIndex && self.originIndex.section == 0)
    {
        ...
        NSString *username = [NSString stringWithFormat:@"%@",[settings objectForKey:@"username"]];
        ...
        self.sectionArray = @[
            @"",
            [NSString stringWithFormat:NSLocalizedString(@"Account (%@)", @""), username],
...
//        }
//    }
    else if (indexPath.section == 3 && self.editMode == YES)
    {
        switch (indexPath.row) {
            case 0:
            {
