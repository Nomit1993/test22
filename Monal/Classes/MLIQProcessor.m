...
{
    if([iqNode check:@"{in:secure:signal:ping}ping"])
    {
...
+(void) processSetIq:(XMPPIQ*) iqNode forAccount:(xmpp*) account
{
    ...
    if(account.connectionProperties.supportsBlocking)
    {
        ...
        if([iqNode check:@"{in:secure:signal:blocking}unblock"])
        {
            NSArray* unBlockItems = [iqNode find:@"{in:secure:signal:blocking}unblock/item@@"];
        ...
        // mark jid as blocked
        if([iqNode check:@"{in:secure:signal:blocking}block"])
        {
            for(NSDictionary* item in [iqNode find:@"{in:secure:signal:blocking}block/item@@"])
            {
                if(item && item[@"jid"])
...
$$handler(handleCatchup, $_ID(xmpp*, account), $_ID(XMPPIQ*, iqNode), $_BOOL(secondTry))
    ...
    {
        DDLogVerbose(@"Paging through mam catchup results with after: %@", [iqNode findFirst:@"{in:secure:signal:mam:2}fin/{http://jabber.org/protocol/rsm}set/last#"]);
...
$$handler(handleMamResponseWithLatestId, $_ID(xmpp*, account), $_ID(XMPPIQ*, iqNode))
    ...
    DDLogVerbose(@"Got latest stanza id to prime database with: %@", [iqNode findFirst:@"{in:secure:signal:mam:2}fin/{http://jabber.org/protocol/rsm}set/last#"]);
    ...
    if([iqNode check:@"{in:secure:signal:mam:2}fin/{http://jabber.org/protocol/rsm}set/last#"])
        [[DataLayer sharedInstance] setLastStanzaId:[iqNode findFirst:@"{in:secure:signal:mam:2}fin/{http://jabber.org/protocol/rsm}set/last#"] forAccount:account.accountNo];
    [account mamFinished];
$$
...
$$handler(handleBind, $_ID(xmpp*, account), $_ID(XMPPIQ*, iqNode))
    ...
    DDLogInfo(@"Now bound to full jid: %@", [iqNode findFirst:@"{in:secure:signal:xml:ns:xmpp-bind}bind/jid#"]);
    [account.connectionProperties.identity bindJid:[iqNode findFirst:@"{in:secure:signal:xml:ns:xmpp-bind}bind/jid#"]];
    ...
    if(account.connectionProperties.supportsSM3)
    {
        MLXMLNode *enableNode = [[MLXMLNode alloc]
            ...
            andNamespace:@"in:secure:signal:sm:3"
...
$$handler(handleAccountDiscoInfo, $_ID(xmpp*, account), $_ID(XMPPIQ*, iqNode))
    ...
    if([features containsObject:@"in:secure:signal:push:0"])
    {
    ...
    if([features containsObject:@"in:secure:signal:mam:2"])
    {
...
$$handler(handleServerDiscoInfo, $_ID(xmpp*, account), $_ID(XMPPIQ*, iqNode))
    ...
    if([features containsObject:@"in:secure:signal:carbons:2"])
    {
        ...
        if(!account.connectionProperties.usingCarbons2)
        {
            ...
            [carbons addChild:[[MLXMLNode alloc] initWithElement:@"enable" andNamespace:@"in:secure:signal:carbons:2"]];
            [account sendIq:carbons withHandler:$newHandler(self, handleCarbonsEnabled)];
        }
    }
    
    if([features containsObject:@"in:secure:signal:ping"])
        account.connectionProperties.supportsPing = YES;
    
    if([features containsObject:@"in:secure:signal:blocking"])
    {
        account.connectionProperties.supportsBlocking = YES;
        [account fetchBlocklist];
    }
    
    if(!account.connectionProperties.supportsHTTPUpload && [features containsObject:@"in:secure:signal:http:upload:0"])
    {
...
$$handler(handleMamPrefs, $_ID(xmpp*, account), $_ID(XMPPIQ*, iqNode))
    ...
    if([iqNode check:@"{in:secure:signal:mam:2}prefs@default"])
        [[MLNotificationQueue currentQueue] postNotificationName:kMLMAMPref object:self userInfo:@{@"mamPref": [iqNode findFirst:@"{in:secure:signal:mam:2}prefs@default"]}];
    else
    {
...
$$handler(handleBlocklist, $_ID(xmpp*, account), $_ID(XMPPIQ*, iqNode))
    ...
    if([iqNode check:@"/<type=result>/{in:secure:signal:blocking}blocklist"])
    {
        NSMutableSet<NSString*>* blockedJids = [[NSMutableSet<NSString*> alloc] init];
        for(NSDictionary* item in [iqNode find:@"/<type=result>/{in:secure:signal:blocking}blocklist/item@@"])
        {
            if(item && item[@"jid"])
            {
